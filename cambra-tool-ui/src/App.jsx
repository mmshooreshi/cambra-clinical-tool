        ...
        <Portal anchorRef={aRef} open={open} onClose={()=>setOpen(false)} maxH={380}>
            <div className="bg-white border-2 border-slate-900 rounded-2xl shadow-[4px_4px_0_#0f172a] overflow-hidden" dir={fa?'rtl':'ltr'}>
                ...
                <div className="grid grid-cols-7 gap-0.5 p-2.5 pt-0.5">
                    {cells.map((day,i)=>{if(!day)return<div key={i}/>;const iT=vY===tY&&vM===tM&&day===tD;const iS=vY===p.y&&vM===p.m&&day===p.d;return(
                        <button key={i} onClick={()=>pk(day)} className={`w-full aspect-square rounded-lg flex items-center justify-center text-xs font-semibold cursor-pointer active:scale-90 transition-all ${iS?'bg-slate-900 text-white shadow-lg':iT?'bg-slate-100 text-slate-900 font-bold ring-2 ring-slate-300':'hover:bg-slate-50 text-slate-600'}`}>{toFa(day,lang)}{iT&&!iS&&<div className="absolute bottom-0.5 w-1 h-1 rounded-full bg-slate-900"/>}</button>
                    )})}
                </div>
            </div>
        </Portal>
...
function FactorRow({label,hint,desc,checked,onToggle,color,weight,lang,descOpen,onDescToggle}){
    ...
    return(
        <div className={`group transition-colors duration-200 ${checked?'bg-slate-50/50':'bg-white'}`}>
            {/* Main row */}
...
function Welcome({lang,onStart}){
    ...
    return(
        <div className="flex flex-col items-center justify-center min-h-[calc(100vh-50px)] px-5 py-10">
            <div className="w-full max-w-md space-y-10 text-center">
                ...
                <div className="c-fade-up c-d1 space-y-3">
                    ...
                    <p className="text-sm text-slate-500 font-medium leading-relaxed max-w-xs mx-auto">
                        {fa
                            ? 'ابزار بالینی ارزیابی ریسک پوسیدگی دندان بر اساس پروتکل CAMBRA. اطلاعات بیمار را وارد کنید، فاکتورها را ثبت کنید و برنامه درمانی دریافت کنید.'
                            : 'Clinical caries risk assessment tool built on the CAMBRA protocol. Enter patient data, record factors, and receive a personalised treatment plan.'
                        }
                    </p>
                </div>
...
export default function CambraApp(){
    ...
    if(!config)return<div className="min-h-screen flex items-center justify-center bg-white"><div className="w-10 h-10 border-[3px] border-slate-200 border-t-slate-900 rounded-full animate-spin"/></div>;

    const t=config.dict[lang];const fa=lang==='fa';const st=config.settings;const isW=tab==='welcome';

    const catBg={extremeRisk:'bg-red-950',highRisk:'bg-red-600',moderateRisk:'bg-amber-500',lowRisk:'bg-emerald-600'};
                ...
                <div className="max-w-2xl mx-auto flex">
                    {hTabs.map(tb=>(
                        <button key={tb.id} onClick={()=>setTab(tb.id)} className={`flex-1 flex flex-col items-center gap-0.5 py-1.5 text-[9px] font-semibold border-b-2 transition-all cursor-pointer ${tab===tb.id?'border-slate-900 text-slate-900':'border-transparent text-slate-400 hover:text-slate-600'}`}>
                            <tb.icon className={`w-3 h-3 ${tab===tb.id?'text-slate-900':'text-slate-300'}`}/>{tb.label}
                        </button>
                    ))}
                </div>
            </div>
        ...
        <main className="max-w-2xl mx-auto p-4 md:p-6">
            ...
            <div className={isW?'hidden':'space-y-5'}>
                ...
                <div className={`${tab!=='disease'?'hidden':''} space-y-5`}>
                    <div className="c-fade-up rounded-2xl border border-slate-200 overflow-hidden bg-white">
                    ...
                    <div className="c-fade-up c-d2 rounded-2xl border border-slate-200 overflow-hidden bg-white">
                ...
                <div className={`${tab!=='results'?'hidden':''} space-y-5 pb-12`}>
                    ...
                    {/* ── Verdict banner ── */}
                    <div className="c-fade-up">
                        <div className={`rounded-2xl overflow-hidden border-2 border-slate-900 shadow-[4px_4px_0_#0f172a] ${catBg[results.finalCat]}`}>
                            {/* Hatching texture */}
                                ...
                                <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-5">
                                    ...
                                    <div>
                                        <div className="flex items-center gap-2 mb-3">
                                            ...
                                            <span className={`text-[8px] font-bold uppercase tracking-[.2em] ${catText[results.finalCat]} opacity-60`}>{fa?'نتیجه ارزیابی':'Assessment Result'}</span>
                                        </div>
            ...
            <nav className="fixed bottom-2 left-1/2 -translate-x-1/2 w-[calc(100%-1rem)] max-w-lg z-40 pointer-events-none">
                <div className="bg-white/95 backdrop-blur-xl border-2 border-slate-900 rounded-xl px-1 py-0.5 flex items-center justify-between gap-1 pointer-events-auto shadow-[3px_3px_0_#0f172a]">
                    ...
                    <div className="flex flex-1 justify-center gap-1">
                        {tabs.filter(t=>t!=='welcome').map((tb,idx)=>{const ci=tabs.indexOf(tab)-1;return(
